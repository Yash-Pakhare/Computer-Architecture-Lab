...
public class Simulator {
	...
	public static void assemble(String objectProgramFile)
	{
		...
		try{
			// Opening the objectProgramFile in binary mode
			...
			// Iterate over instructions and assemble one instruction at a time, and write to the file
			for(int i=0; i<ParsedProgram.code.size(); i++){
				...
				// For add, sub, mul,..., sra instructions
				else if(Integer.parseInt(opTable.get(opType), 2)%2==0 && Integer.parseInt(opTable.get(opType), 2)<=21){
					binInst += (opTable.get(opType));
					if(ParsedProgram.code.get(i).sourceOperand1.operandType.toString().equals("Register")){
						binInst = binInst.concat(String.format("%5s", Integer.toBinaryString(ParsedProgram.code.get(i).sourceOperand1.value)).replace(' ', '0'));
					}
				...
				// For addi, subi, muli,..., srai instructions
				else if(Integer.parseInt(opTable.get(opType), 2)%2!=0 && Integer.parseInt(opTable.get(opType), 2)<=21){
					binInst += (opTable.get(opType));
					if(ParsedProgram.code.get(i).sourceOperand1.operandType.toString().equals("Register")){
						binInst = binInst.concat(String.format("%5s", Integer.toBinaryString(ParsedProgram.code.get(i).sourceOperand1.value)).replace(' ', '0'));
				...
				// For beq, bne, blt, bgt instructions
				else if(Integer.parseInt(opTable.get(opType), 2) >= 25 && Integer.parseInt(opTable.get(opType), 2) <= 28){
					binInst += (opTable.get(opType));
					if(ParsedProgram.code.get(i).sourceOperand1.operandType.toString().equals("Register")){
						binInst = binInst.concat(String.format("%5s", Integer.toBinaryString(ParsedProgram.code.get(i).sourceOperand1.value)).replace(' ', '0'));
					}
