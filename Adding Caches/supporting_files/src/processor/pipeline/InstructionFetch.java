...
public class InstructionFetch implements Element{
		...
		if(IF_EnableLatch.isIF_enable())
		{
			...
			if(EX_IF_Latch.getIF_enable()){
				...
				if(IF_OF_Latch.isBranchHazard()){
					...
					// Memory read latency
					Simulator.getEventQueue().addEvent(new MemoryReadEvent(Clock.getCurrentTime(), this, containingProcessor.getL1iCache(), currentPC));
					...
					System.out.println("hi i got the new branch address and event was added with the  address: "+currentPC);
	...
	public void handleEvent(Event e) {
		...
		else
		{
			...
			// IF_EnableLatch.setIF_busy(true);
			// IF_OF_Latch.setOF_enable(false);
			System.out.println("pc IF event handeled for this at clock: "+Clock.getCurrentTime()+"  "+ containingProcessor.getRegisterFile().getProgramCounter());
			String newBinaryInstruction = Integer.toBinaryString(newInstruction);
			newBinaryInstruction = String.format("%32s", newBinaryInstruction).replace(' ', '0');
			containingProcessor.getRegisterFile().setProgramCounter(currentPC + 1);
			IF_OF_Latch.setInstruction(newBinaryInstruction);
