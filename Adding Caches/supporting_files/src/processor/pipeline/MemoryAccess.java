	...
	public void performMA()
	{
		...
		if(EX_MA_Latch.isMA_enable())
		{
			...
			if(binaryInstruction!=null)
			{
				...
				switch (operation) {
					case load:
						Simulator.getEventQueue().addEvent(new MemoryReadEvent(Clock.getCurrentTime(), this, containingProcessor.getL1dCache(), aluresult));
						...
						break;
					case store:
						Simulator.getEventQueue().addEvent(new MemoryWriteEvent(Clock.getCurrentTime(), this, containingProcessor.getL1dCache(), aluresult, containingProcessor.getRegisterFile().getValue(rs1)));
						...
						IF_OF_Latch.setOF_busy_MA(true);
						MA_RW_Latch.setRW_busy(true);
						System.out.println("Store DETECTED and inserted into the queue time: "+Clock.getCurrentTime());
